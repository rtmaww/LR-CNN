...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        if not instance:
            continue
        gaz_list,batch_word, batch_biword, batch_wordlen, batch_label, layer_gaz, gaz_mask, mask  = batchify_with_label(instance, data.HP_gpu, data.HP_num_layer, True)
...
def batchify_with_label(input_batch_list, gpu, num_layer, volatile_flag=False):
    ...
    for idx, (seq, biseq, label, seqlen, layergaz, gazmask) in enumerate(zip(words, biwords, labels, word_seq_lengths, layer_gazs, gaz_mask)):
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            if not instance:
                continue
            gaz_list,  batch_word, batch_biword, batch_wordlen, batch_label, layer_gaz, gaz_mask, mask  = batchify_with_label(instance, data.HP_gpu,data.HP_num_layer)

...
def read_instance_with_gaz(num_layer, input_file, gaz, word_alphabet, biword_alphabet, char_alphabet, gaz_alphabet, label_alphabet, number_normalized, max_sent_length, char_padding_size=-1, char_padding_symbol = '</pad>'):
    ...
    for idx in range(len(in_lines)):
        ...
        if len(line) > 2:   
            ...
            if ((max_sent_length < 0) or (len(words) < max_sent_length)) and (len(words)>0):  
                ...
                instence_Ids.append([word_Ids, biword_Ids, char_Ids, gaz_Ids, label_Ids, layergazs, layergazmasks])
            words = []
            biwords = []
            chars = []
            labels = []
